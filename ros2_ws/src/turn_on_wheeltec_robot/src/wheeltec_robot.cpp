...
static bool velocity_nonzero(double linear_x, double linear_y, double angular_z, double threshold)
{
  return fabs(linear_x) > threshold || fabs(linear_y) > threshold || fabs(angular_z) > threshold;
}
...
{
  geometry_msgs::msg::Twist avoid_twist_cached = avoid_obstacle(*ori_twist_aux);
  current_linear_x = clamp_double(avoid_twist_cached.linear.x, -max_linear_x, max_linear_x);
  current_linear_y = clamp_double(avoid_twist_cached.linear.y, -max_linear_y, max_linear_y);
  current_angular_z = clamp_double(avoid_twist_cached.angular.z, -max_angular_z, max_angular_z);
  last_cmd_time = this->get_clock()->now();
  has_received_cmd = true;
}


void turn_on_robot::Cmd_Send_Timer_Callback()
{
  double vx = 0.0;
  double vy = 0.0;
  double wz = 0.0;
  ...
  if(has_received_cmd)
  {
    ...
    if(age <= cmd_timeout_sec)
    {
      vx = current_linear_x;
      vy = current_linear_y;
      wz = current_angular_z;
    }
  }

  if(velocity_nonzero(vx, vy, wz, brake_trigger_threshold))
  {
    if(!motion_active)
    {
    ...
    last_motion_linear_x = vx;
    last_motion_linear_y = vy;
    last_motion_angular_z = wz;
    Send_Cmd_Frame(vx, vy, wz);
    return;
  }
  ...
  if(motion_active)
  {
    motion_duration_sec = (now - motion_start_time).seconds();
    motion_active = false;
  }

  if(braking_active)
  {
    if(now < brake_end_time)
    {
      Send_Cmd_Frame(brake_linear_x, brake_linear_y, brake_angular_z);
      return;
    }
    braking_active = false;
  }
  ...
  if(brake_enable &&
     ...
     motion_duration_sec >= brake_min_motion_sec &&
     velocity_nonzero(last_motion_linear_x, last_motion_linear_y, last_motion_angular_z, brake_trigger_threshold))
  {
    double effective_brake_duration = brake_duration_sec;
    if(brake_duration_gain > 0.0)
    {
      effective_brake_duration = std::min(brake_duration_sec, motion_duration_sec * brake_duration_gain);
    }
    brake_linear_x = clamp_double(-last_motion_linear_x * brake_ratio, -max_linear_x, max_linear_x);
    brake_linear_y = clamp_double(-last_motion_linear_y * brake_ratio, -max_linear_y, max_linear_y);
    brake_angular_z = clamp_double(-last_motion_angular_z * brake_ratio, -max_angular_z, max_angular_z);
    brake_end_time = now + rclcpp::Duration::from_nanoseconds(
        static_cast<int64_t>(effective_brake_duration * 1000000000.0));
    braking_active = true;
    last_motion_linear_x = 0.0;
    last_motion_linear_y = 0.0;
    last_motion_angular_z = 0.0;
    Send_Cmd_Frame(brake_linear_x, brake_linear_y, brake_angular_z);
    return;
  }
...
void turn_on_robot::Send_Cmd_Frame(double linear_x, double linear_y, double angular_z)
{
  short transition;
  ...
  transition = static_cast<short>(linear_x * 1000.0);
  Send_Data.tx[4] = transition;
  Send_Data.tx[3] = transition>>8;

  transition = static_cast<short>(linear_y * 1000.0);
  Send_Data.tx[6] = transition;
  Send_Data.tx[5] = transition>>8;

  transition = static_cast<short>(angular_z * 1000.0);
...
turn_on_robot::turn_on_robot():rclcpp::Node ("wheeltec_robot")
{
  ...
  this->declare_parameter<double>("cmd_timeout_sec",0.25);
  this->declare_parameter<double>("max_linear_x",0.45);
  this->declare_parameter<double>("max_linear_y",0.0);
  this->declare_parameter<double>("max_angular_z",2.4);
  this->declare_parameter<bool>("brake_enable",true);
  this->declare_parameter<double>("brake_ratio",1.0);
  this->declare_parameter<double>("brake_duration_sec",0.5);
  this->declare_parameter<double>("brake_trigger_threshold",0.02);
  this->declare_parameter<double>("brake_duration_gain",1.0);
  this->declare_parameter<double>("brake_min_motion_sec",0.12);
  ...
  this->get_parameter("send_rate_hz", send_rate_hz);
  this->get_parameter("cmd_timeout_sec", cmd_timeout_sec);
  this->get_parameter("max_linear_x", max_linear_x);
  this->get_parameter("max_linear_y", max_linear_y);
  this->get_parameter("max_angular_z", max_angular_z);
  this->get_parameter("brake_enable", brake_enable);
  this->get_parameter("brake_ratio", brake_ratio);
  this->get_parameter("brake_duration_sec", brake_duration_sec);
  this->get_parameter("brake_trigger_threshold", brake_trigger_threshold);
  this->get_parameter("brake_duration_gain", brake_duration_gain);
  this->get_parameter("brake_min_motion_sec", brake_min_motion_sec);
  ...
  */
  ...
  auto send_period = std::chrono::duration<double>(1.0 / send_rate_hz);
  cmd_send_timer_ = this->create_wall_timer(
      std::chrono::duration_cast<std::chrono::nanoseconds>(send_period),
      std::bind(&turn_on_robot::Cmd_Send_Timer_Callback, this));
  RCLCPP_INFO(this->get_logger(),
      "cmd watchdog enabled: send_rate=%.2fHz timeout=%.3fs max_vx=%.3f max_vy=%.3f max_wz=%.3f",
      send_rate_hz, cmd_timeout_sec, max_linear_x, max_linear_y, max_angular_z);
  RCLCPP_INFO(this->get_logger(),
      "brake pulse: enable=%d ratio=%.3f max_duration=%.3fs gain=%.3f min_motion=%.3fs threshold=%.3f",
      brake_enable, brake_ratio, brake_duration_sec, brake_duration_gain,
      brake_min_motion_sec, brake_trigger_threshold);
}
/**************************************
Date: January 28, 2021
